...
function OverviewMockup({ t }: Props) {
  const rows = [
    { repo: 'web-app', branch: 'main', ahead: 2, behind: 1, status: t.mockup.synced, dot: 'bg-emerald-500', text: 'text-emerald-600' },
    { repo: 'mobile-api', branch: 'feature/payments', ahead: 5, behind: 0, status: t.mockup.modified, dot: 'bg-amber-500', text: 'text-amber-600' },
    { repo: 'infra', branch: 'master', ahead: 0, behind: 3, status: t.mockup.synced, dot: 'bg-emerald-500', text: 'text-emerald-600' },
    { repo: 'docs', branch: 'main', ahead: 1, behind: 4, status: t.mockup.conflict, dot: 'bg-red-500', text: 'text-red-600' },
  ];
  ...
  return (
    <div className="glass-card overflow-hidden">
      ...
      <div className="flex">
        <aside className="hidden w-32 shrink-0 flex-col gap-0.5 border-r border-slate-200/70 bg-white/30 p-2 sm:flex">
          {t.mockup.sidebar.map((item, i) => (
            <span
              key={item}
              className={`rounded-md px-2.5 py-1.5 font-mono text-[10px] uppercase tracking-[0.14em] ${
                i === 0 ? 'bg-cyan-500/10 font-semibold text-primary' : 'text-slate-400'
              }`}
            >
              {item}
            </span>
          ...
          <div className="grid grid-cols-[1fr_auto_auto_auto] gap-x-4 gap-y-2 text-[11px]">
            ...
            {rows.map((row) => (
              <div key={row.repo} className="contents">
                <span className="truncate font-mono text-[11px] text-slate-800">{row.repo}</span>
                <span className="truncate font-mono text-[11px] text-slate-400">{row.branch}</span>
                <span className="text-right font-mono text-[11px] text-slate-500">{row.ahead}</span>
                <span className="text-right font-mono text-[11px] text-slate-500">{row.behind}</span>
              </div>
            ))}
          </div>
      ...
      <div className="flex items-center gap-3 border-t border-slate-200/70 bg-white/30 px-4 py-2">
        ...
        <span className="ml-auto rounded border border-slate-200 bg-white/60 px-1.5 py-0.5 font-mono text-[10px] text-slate-500">
          ctrl+1
        </span>
      </div>
    </div>
...
function TelemetryCard() {
  const rows = [
    { label: 'repos', value: '12', dot: 'bg-emerald-500' },
    { label: 'changes', value: '3', dot: 'bg-amber-500' },
    { label: 'conflicts', value: '1', dot: 'bg-red-500' },
  ];
  return (
      ...
      <div className="mt-2.5 space-y-1.5">
        {rows.map((row) => (
          <div key={row.label} className="flex items-center gap-2 font-mono text-[10px]">
            ...
            <span className="ml-auto text-slate-800">{row.value}</span>
          </div>
        ))}
      </div>
    </div>
...
function LogCard() {
  const lines = [
    { tag: 'ok', text: 'fetch web-app (1.2s)', color: 'text-emerald-600' },
    { tag: 'ok', text: 'pull mobile-api (2.4s)', color: 'text-emerald-600' },
    { tag: 'err', text: 'push docs — rejected', color: 'text-red-500' },
  ];
  return (
      ...
      <div className="container max-w-6xl px-4 md:px-6">
        <div className="mx-auto grid max-w-6xl items-center gap-14 lg:grid-cols-[1.05fr_1fr] lg:gap-10">
          <div className="text-center lg:text-left">
            <p className="mono-label inline-flex items-center gap-2.5 rounded-full border border-slate-200/80 bg-white/60 px-3.5 py-1.5 shadow-sm backdrop-blur">
            ...
            <div className="mt-9 flex flex-col items-center justify-center gap-3 sm:flex-row lg:justify-start">
              <a
                href="#download"
                className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-primary px-7 py-3.5 text-sm font-semibold text-white shadow-xl shadow-cyan-600/25 transition-all hover:bg-cyan-700 hover:shadow-cyan-600/35 sm:w-auto"
              >
          ...
          <div className="relative mx-auto w-full max-w-xl lg:max-w-none">
            <div
              className="pointer-events-none absolute -inset-10 rounded-full bg-cyan-400/20 blur-[90px]"
              aria-hidden
            />
            <OverviewMockup t={t} />
