...
- 修复：macOS 毛玻璃只剩半透明——Skia 采样被关掉且原生 vibrancy 被整窗壁纸挡住；改为 CPU 预模糊壁纸底板（ADR-0038）
- 修复：实时模式弹窗无模糊——原局部绘制的整窗 `DeviceClipBounds` 检查永不成立且截图时机在遮罩之后；改为 `DialogBackdropCapture` 在新弹窗/遮罩出现前完成整窗截取与一次高斯模糊，`FrozenBackdropPlate` 仅在弹窗面板内按位置采样，拖动只换矩阵，失败时回退到面板局部模糊且绝不显示未模糊原图
- 优化：壁纸底板质量——`ResolveWallpaperFactor`/`ResolveDialogCaptureFactor` 限 factor ≤ 4，`WallpaperBackdrop.RenderBlurred` 改用 `SKCubicResampler.Mitchell` 分阶段减半（factor 2 单步、factor 4 两步）并对奇数尺寸向上取整，消除线性一次缩放到 factor 8 的马赛克
- 优化：壁纸模式玻璃染色——新增 `MggChrome/Card/ToastGlassBrushWallpaperMode` 并用更具体的 `Window.wallpaperBackdrop.visualEffects` 规则覆盖 toolbar/statusbar/sidebarRail/card/toastCard，现有 dialog/loading/popover/tooltip 调为冷色（Light `#99E3F3F8` 等、Dark `#8C12344A` 等），titleBar 保持透明
- 优化：Windows/Linux 外观设置增加「实时模糊」；关闭后与 macOS 一样用预模糊壁纸
- 优化：模态对话框打开时截一次整窗底板，拖动只换取样区域，不再每帧重模糊
