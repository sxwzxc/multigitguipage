...
- **IP 与地理位置**：Edge Functions 内置 `request.eo` —— `eo.clientIp` 为真实客户端 IP，
  `eo.geo` 提供国家代码（`countryCodeAlpha2`，ISO 3166-1 alpha-2）/国家名/省份/城市，**无需控制台配置**；
  回退链为 `X-Forwarded-For` 首个值 / `EO-Connecting-IP`（回源场景）、`EO-Client-IPCountry`
  （规则引擎「客户端 IP 地理位置」操作）。记录含 `ip`、`country`、`location`（如 `China · Guangdong · Shenzhen`）。
